...
test('visit', async ({ page }) => {
    ...
    // Navigate to the application's main page before interacting with elements
...
test('exisitng patient', async ({ page }) => {
    ...
    await page.waitForTimeout(10000);
    await expect (page.locator('Search for patients...')).toHaveText("QA ")
    await expect(page.getByText('Last-visit Summary')).toBeVisible();
...
test('Valid Transcribed', async ({ page }) => {
    ...
    await expect(transcript).toContain(Coughtext);
...
test('Televisit', async ({ page }) => {
    ...
    const fileInput= await page.getByText('Click or drag file to this area to upload').click();
